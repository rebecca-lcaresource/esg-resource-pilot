...
## Known issues
- **OPEN — production control read path is broken (found 21 Sep 2026).** Migration `set_security_invoker_on_suppliers_pc` (26 Aug 2026, not recorded in this repo) changed the `suppliers_pc` view to `security_invoker=on`. The view now runs as the caller, hits base-table RLS, and production control — which deliberately has no SELECT policy on `suppliers` — gets **0 rows** (verified by impersonation). Breaks AC 3, 8 and 10 for that role and the core demo. Almost certainly done to silence the Supabase `security_definer_view` advisor, which this file already lists as an accepted exception. Fix is one statement: `alter view public.suppliers_pc set (security_invoker = off);` — awaiting builder go-ahead. docs/supabase-setup.md still describes the intended (definer) state.
...
- FIRST: restore `suppliers_pc` to a definer view (see Known issues) once the builder confirms — then re-run docs/authorization-proof.md A1/A2 to show production control sees 14 rows via the view and 0 via the table. Also check whether the same advisor-driven change touched anything else on 26 Aug (only one migration is listed for that date, but check `profiles`/`change_log` grants too).
