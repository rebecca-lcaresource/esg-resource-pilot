...
| 8 | Production control cannot see restricted columns **in the payload** | Inspecting the network response as a production control user reveals no justification, internal notes, contract status, renewal date or annual spend anywhere | [ ] DB side verified 21 Sep 2026: `suppliers_pc` exposes exactly `id, supplier_name, country, category, esg_report_url, score_e, score_s, score_g, overall_score, updated_at`; production control reads 0 rows from the base table. Network-payload inspection in the app still to do |
| 9 | Production control cannot write | Any insert, update or archive attempt is rejected by the database | [x] 21 Sep 2026 — insert: `new row violates row-level security policy`; update, archive and delete with no WHERE: 0 rows affected |
...
| 12 | Change log is filtered by role | Production control sees only entries for fields they may read; no entry reveals a restricted field name or value | [x] 21 Sep 2026 — production control sees 8 of 10 log rows; the 2 hidden are `score_justification` and `is_archived` |
| 13 | Change log cannot be altered | Update and delete attempts on `change_log` fail for every role, including admin | [x] 21 Sep 2026 — admin UPDATE and DELETE on change_log both `permission denied for table change_log` (refused at GRANT layer, before RLS) |
...
| 15 | Only admin can permanently delete | Delete control is unavailable to other roles and refused at the database. After deletion, the supplier's change log rows remain and remain readable | [x] 21 Sep 2026 (DB side) — sustainability and purchasing DELETE: 0 rows; admin DELETE unarchived: 0 rows; admin DELETE after archive: 1 row; all 11 change_log rows retained with `supplier_id` set null and `supplier_name_snapshot` intact |
| 16 | User Management is admin-only at the database level | A non-admin requesting profile data for other users is refused by RLS, not just redirected | [x] 21 Sep 2026 — purchasing SELECT and UPDATE on other profiles: 0 rows; admin UPDATE on other profiles: 4 rows |
